...
"""
    ...
    def test_7a_plus(self):
        ...
        for test_case in test_cases:
            ...
            self.assertTrue(isinstance(pvm + test_case[-1], Paivays), f'Metodin __add__ pitäisi palauttaa uusi Paivays-tyyppinen olio\n' +
                f'Tarkasta metodi {met_name}')
    ...
    def test_7b_plus(self):
        ...
        for test_case in test_cases:
            pvm = Paivays(test_case[0], test_case[1], test_case[2])
            alkup_pvm = f'{test_case[0]}.{test_case[1]}.{test_case[2]}'
            val = pvm + test_case[-1]
            koodi = f"pvm = Paivays({test_case[0]}, {test_case[1]}, {test_case[2]})\npvm2 = pvm + {test_case[-1]}"

            self.assertEqual(alkup_pvm, str(pvm), f'Päivämäärän kasvattaminen ei saa muuttaa alkuperäistä päiväystä.' +
                f'\nSeuraavan koodin suorittamisen jälkeen:\n{koodi}\n' +
                f'alkuperäisen päiväyksen tulisi olla {alkup_pvm}, se kuitenkin on {pvm}\n' + '')
